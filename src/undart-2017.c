...
echo gcc -g $M -Wall -Werror -o $BIN/$NAME $HOME/${NAME}-2017.c $HOME/md5.c
time gcc -g $M -Wall -Werror -o $BIN/$NAME $HOME/${NAME}-2017.c $HOME/md5.c
return
#endif
 ...
 * 2017 notes
 * =======================================================================================
May 2017 on Ubuntu 16.04.1 LTS
bgb@work9$ uname -a
Linux work9 4.4.0-64-generic #85-Ubuntu SMP Mon Feb 20 11:50:30 UTC 2017 x86_64 x86_64 x86_64 GNU/Linux
#
# 
#

`2017-05-02 bgbaumgart@mac.com'

`2016-09-09 bgbaumgart@mac.com'
 * =======================================================================================
 * 2016 notes
 * =======================================================================================
September 2016 on Ubuntu 16.04.1 LTS
uname -a
Linux work8 4.4.0-36-generic #55-Ubuntu SMP Thu Aug 11 18:01:55 UTC 2016 x86_64 x86_64 x86_64 GNU/Linux

bgb@work8$ mkdir -p /m/2016/{csv,dart,data13/sn,data7/sn,log,tbz}
bgb@work8$ chown bgb:bgb -R /m/2016
...
#
# create static web site into Lcorpus
#
 ...
 * -------------------------------------------------------------------- *
INPUT file paths:
        /data/DARTremix./sn.tmp
        /data/DARTremix./hash.tmp        
OUTPUT file paths:
        /large/log/undart
        /large/csv/undart-tag-metadata
        /large/csv/undart-blob-sn-hash
 */
int
main(int ac,char **av){
  int ma, record_number, arg;
  {
    ...
    size_t len=16;
    FILE *F;
    assert(F = fopen("/data/DARTremix./sn.tmp","r"));
    while(getline(&buf,&len,F)>0){
      val[i++] = strtol(buf,0,5+5);
    }
    fclose(F);
    i=0;
    assert(F = fopen("/data/DARTremix./hash.tmp","r"));
   ...
   * initialization 
   */
  errno=0;  stdlog = fopen("/large/log/undart","w");  assert(errno==0);
  errno=0;  stdcsv = fopen("/large/csv/undart-tag-metadata","w");  assert(errno==0);
  errno=0;  snhash = fopen("/large/csv/undart-blob-sn-hash","w"); assert(errno==0);

